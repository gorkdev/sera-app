...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (form.dataset.ajaxSubmit === 'true') {
        form.addEventListener('submit', async (e) => {
            ...
            if (submitBtn) {
                ...
                submitBtn.innerHTML = '<span class="loading loading-spinner loading-sm"></span> Gönderiliyor...';
            }
    ...
    function showFormErrors(form, errors) {
        ...
        form.querySelectorAll('.input-error, .select-error, .textarea-error').forEach(el => el.classList.remove('input-error', 'select-error', 'textarea-error'));

        const tabFields = {
            genel: ['category_id', 'name', 'slug', 'sku', 'description', 'image', 'gallery_images'],
            fiyat: ['cost_price', 'price', 'unit', 'unit_conversions'],
            stok: ['stock_quantity', 'min_order_quantity', 'critical_stock_type', 'critical_stock_value', 'critical_stock_reference'],
            ozellikler: ['featured_badges', 'origin', 'shelf_life_days'],
            durum: ['is_active'],
        };

        const addedToContainer = { gallery: false, unit: false };
        ...
        for (const [field, messages] of Object.entries(errors)) {
            ...
            if (field.startsWith('gallery_images')) {
                ...
                if (formControl && !addedToContainer.gallery) {
                    ...
                    formControl.querySelectorAll('.file-input').forEach(el => el.classList.add('input-error'));
                }
                continue;
            }
        ...
        Object.keys(tabFields).forEach(tab => {
            ...
            const hasError = Object.keys(errors).some(key => fields.some(f => key === f || key.startsWith(f + '.')));
    ...
    function scrollToFirstError(form) {
        ...
        if (first) first.closest('section')?.scrollIntoView({ behavior: 'smooth', block: 'center' });
    }
    ...
    if (addGalleryBtn && galleryContainer) {
        ...
        if (!document.getElementById('gallery-existing') && galleryContainer.querySelectorAll('.gallery-row').length === 0) {
            addGalleryRow();
        }
    ...
    if (costInput && priceInput) {
        ...
        priceInput.addEventListener('input', () => { priceManuallyEdited = true; });
    }
