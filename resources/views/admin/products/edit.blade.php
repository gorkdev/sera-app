...
@section('content')
    ...
    <form method="POST" action="{{ route('admin.products.update', $product) }}" class="admin-form space-y-6 max-w-3xl"
        ...
        data-errors='@json($errors->keys())' data-ajax-submit="true"
...
(function() {
    ...
    function doScroll() {
        ...
        if (flash) flash.scrollIntoView({ behavior: 'instant', block: 'start' });
    }
    ...
    window.addEventListener('pageshow', function(ev) {
        if (ev.persisted && new URLSearchParams(window.location.search).get('scroll') === 'top') doScroll();
    });
})();
</script>
@endpush
