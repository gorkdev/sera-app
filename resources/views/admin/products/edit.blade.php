...
@section('content')
    ...
    <form method="POST" action="{{ route('admin.products.update', $product) }}" class="admin-form space-y-6 max-w-3xl"
        enctype="multipart/form-data" id="product-form" data-units='@json($productUnits)'
        data-unit-conversions='@json($unitConversions)' data-existing-gallery="{{ count($existingGallery) }}"
        data-errors='@json($errors->keys())'>
        @csrf
        @method('PUT')
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="form-control">
                            ...
                            <textarea id="description" name="description" rows="4"
                                ...
                                placeholder="Ürün açıklaması (en az 10 karakter)...">{{ old('description', $product->description) }}</textarea>
                            @error('description')
                                <p class="text-error text-sm mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="form-control">
                            <label class="label"><span class="label-text font-medium">Ana Görsel <span
                                        class="text-error">*</span></span><span class="label-text-alt">Max 2MB — mevcut
                                    varsa değiştirmek opsiyonel</span></label>
                            @if ($product->image)
                        ...
                        <div class="form-control">
                            <label class="label"><span class="label-text font-medium">Alt Görseller <span
                                    ...
                                    3</span></label>
                            <div id="gallery-existing" class="flex flex-wrap gap-3 mb-2">
                                @foreach ($existingGallery as $idx => $path)
                                    <div class="gallery-existing-item flex items-center gap-2">
                                        <img src="{{ Storage::url($path) }}" alt=""
                                            class="h-14 w-14 object-cover rounded border border-base-300" />
                                        <label class="flex items-center gap-1 text-sm text-error cursor-pointer">
                                            <input type="checkbox" name="gallery_remove[]" value="{{ $idx }}"
                    ...
                    <div class="form-control mb-6">
                        ...
                        <select name="unit" id="unit"
                            ...
                            @foreach ($productUnits as $key => $label)
                                <option value="{{ $key }}"
                                    {{ old('unit', $product->unit) == $key ? 'selected' : '' }}>{{ $label }}
                                </option>
                            @endforeach
                        </select>
                        @error('unit')
                ...
                <section class="admin-form-section">
                        ...
                        <div class="form-control">
                            ...
                            <select name="critical_stock_type" id="critical_stock_type" class="select select-md w-full">
                                ...
                                <option value="percent"
                                    {{ old('critical_stock_type', $product->critical_stock_type) == 'percent' ? 'selected' : '' }}>
                                    Yüzde (%)</option>
                                <option value="quantity"
                                    {{ old('critical_stock_type', $product->critical_stock_type) == 'quantity' ? 'selected' : '' }}>
                                    Adet</option>
                            </select>
                        </div>
                        <div class="form-control">
                            ...
                            <input type="number" id="critical_stock_value" name="critical_stock_value"
                                value="{{ old('critical_stock_value', $product->critical_stock_value) }}"
                                class="input input-bordered input-md w-full" min="0" placeholder="10" />
                        </div>
                    </div>
                    <div class="form-control mt-4" id="critical-ref-wrap" style="display:none">
                        ...
                        <input type="number" id="critical_stock_reference" name="critical_stock_reference"
                            value="{{ old('critical_stock_reference', $product->critical_stock_reference ?? 100) }}"
                                    ...
                                    @foreach ($featuredBadges as $key => $label)
                                        <label class="flex items-center gap-2 cursor-pointer">
                                            <input type="checkbox" name="featured_badges[]" value="{{ $key }}"
                                                class="checkbox checkbox-sm"
                                                {{ in_array($key, old('featured_badges', $product->featured_badges ?? [])) ? 'checked' : '' }} />
                            ...
                            <div class="form-control">
                                ...
                                <select name="origin" id="origin" class="select select-md w-full max-w-xs">
                                    ...
                                    @foreach ($originOptions as $key => $label)
                                        <option value="{{ $key }}"
                                            {{ old('origin', $product->origin) == $key ? 'selected' : '' }}>
                                            {{ $label }}</option>
                                    @endforeach
                                </select>
                            </div>
